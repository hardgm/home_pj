...
unsigned long buzzerStartTime = 0;  // 버저 시작 시간 기록
unsigned long buzzerDuration = 500; // 버저가 울리는 시간 (500ms)
bool buzzerActive = false;          // 버저 활성 상태
...
void activateBuzzer() {
  if (!buzzerActive) {
    ...
    */
    ...
    buzzerStartTime = millis();  // 시작 시간 기록
    buzzerActive = true;         // 활성 상태 설정
  }
}
  ...
  if (mySerial.available()>0) {
    ...
    if (millis() - lastMeasurementTime >= measurementInterval) {
      ...
      if (millis() - lastTriggerTime >= triggerInterval) {
        lastTriggerTime = millis();  // 초음파 송신 타이밍 기록
        triggerStartTime = micros(); // 트리거 시작 시간 기록

        // 초음파 송신 (트리거)
        digitalWrite(PIN_TRIG, HIGH); // 트리거 핀 HIGH
      }

      // 트리거 핀을 HIGH로 설정하고 20마이크로초 후 LOW로 설정
      if (micros() - triggerStartTime >= triggerDuration) {
          digitalWrite(PIN_TRIG, LOW);  // 트리거 핀 LOW
      }
      */
